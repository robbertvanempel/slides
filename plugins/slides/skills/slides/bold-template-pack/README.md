...
This pack brings bundled bold design systems into the `slides` skill without
making them the default for every deck.
...
If the wildcard is custom, it must follow Slides' no-slop aesthetics:
...
- Keep `slides` output as one self-contained HTML file.
